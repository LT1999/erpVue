          ...
          <el-table :data="tableData"  style="width: 100%">
            ...
            <el-table-column label="入库调度">
              <template slot-scope="scope">
                <el-button size="mini" type="success" @click="handleEdit(scope.$index,scope.row)">入库调度</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      selectcheckTag(){
        this.$http.post("http://localhost:8080/Erp-web/gather/selectcheckTag.do")
        	.then( res => {
      ...
      handleEdit(index,row){
        this.$router.push({
        	path: '/delivery2',
					query:{id:row.id,gatherId:row.gatherId,storer:row.storer,register:row.register,remark:row.remark,
          reason:row.reason,registerTime:row.registerTime,amountSum:row.amountSum,costPriceSum:row.costPriceSum,gatheredAmountSum:row.gatheredAmountSum}
        });
      }
    }
  }
